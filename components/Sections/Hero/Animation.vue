...
const tags = [...data, ...resources, ...programs, ...stages]
const tagsforMobile = [...data, ...resources, ...programs]

// Positions for animation
const positions = reactive(tags.map(() => ({ x: 0, y: 0 })))
const targetOffsets = reactive(tags.map(() => ({ x: 0, y: 0 })))
...
function updatePositions() {
  tagRefs.value.forEach((tagEl, index) => {
    if (!tagEl) return
    positions[index].x += (targetOffsets[index].x - positions[index].x) * 0.1
    positions[index].y += (targetOffsets[index].y - positions[index].y) * 0.1
...
function handleMouseMove(e: MouseEvent) {
  ...
  tagRefs.value.forEach((_, index) => {
    const coeff = movementCoefficients[index]
...
<template>
  <div ref="containerRef">
    <UIFadeUpAnimation :delay-index="0" class="mb-4">
      <div class="relative">
        <!-- Background image -->
        <div
        ...
        <SectionsHeroContent class="block xl:hidden">
          ...
          <template #contentTags>
            ...
            >
              ...
              <SectionsHeroTag
                v-for="(tag, index) in tagsforMobile"
                :key="index"
                :tag="tag"
                :isTag="showTags"
              />
